...
$dosya= fopen("uploads/108591026siniflar_json.txt", "r") or die("Dosya bulunamadı!");
$json_data = fread($dosya,filesize("uploads/108591026siniflar_json.txt"));
fclose($dosya);

 
$data = json_decode($json_data);
echo "<pre>";
$_SESSION['varolan_siniflar']['no'] = [];
foreach ($data as $datalar) {

	$query = $db->prepare("SELECT * FROM sinif WHERE sinif_no =:sinif_no"); 
	$query->execute(array( 
		'sinif_no' => $datalar->sinif 
	));
	if ($query->rowCount() < 1) {
		
		$ayarkaydet = $db->prepare("INSERT INTO sinif SET 
		sinif_no=:sinif_no,
		sinif_kapasite=:sinif_kapasite
		"); 
		$update = $ayarkaydet->execute(array(
		'sinif_no' =>  $datalar->sinif,
		'sinif_kapasite' =>   $datalar->kapasite
		));

	}else{
		array_push($_SESSION['varolan_siniflar']['no'] , $datalar->sinif); 
	}
}
 
$_SESSION['bilgi'][1] = 1;
$_SESSION['bilgi'][0] = "Json Başarıyla İçe Aktarıldı";
header("Location:../sinif-aktar.php");
/*
